...
async def evaluate_resume(
    ...
    request_hash = compute_request_hash(resume_bytes, jd)
    existing = await db.execute(
        select(Evaluation).where(Evaluation.jd_text == jd).limit(10)
    )
    evaluations = existing.scalars().all()
    for eval in evaluations:
        if eval.status == "completed" and eval.resume_filename == resume.filename:
            logger.info(
                f"Returning existing evaluation for duplicate request: {eval.id}"
            )
            return eval
